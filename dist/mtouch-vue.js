(function (global, factory) {
  ...
  (global = global || self, global.mTouch = factory());
  ...
  var eventList = {
    TOUCH_START: util$1.hasTouch ? 'touchstart' : 'mousedown',
    TOUCH_MOVE: util$1.hasTouch ? 'touchmove' : 'mousemove',
    TOUCH_END: util$1.hasTouch ? 'touchend' : 'mouseup',
  ...
  function bindTouchEvents(el) {
    ...
    var triggerSingleTap = function triggerSingleTap(event) {
      ...
      util$1._trigger(eventList.TAP, el, event);
    }; // 滑动被取消
    ...
    var cancel = function cancel(event) {
      ...
      util$1._trigger(eventList.CANCEL, el, event);
    }; // 开始触屏监听函数


    var touchstart = function touchstart(event) {
      var touch = util$1.hasTouch ? event.touches[0] : event;
      x1 = touch.pageX;
      y1 = touch.pageY;
      x2 = 0;
      y2 = 0;
      ...
      util$1._trigger(eventList.SWIPE_START, el, event);
      ...
      longTapTimer = setTimeout(function () {
        ...
        util$1._trigger(eventList.LONG_TAP, el, event);
    ...
    var touchmove = function touchmove(event) {
      ...
      moveDirection = util$1.swipeDirection(oldX2, oldY2, x2, y2, directionLocked);
      ...
      util$1._trigger(eventList.SWIPING, el, event); // 滑动时差超过300ms时，重置一下开始时间
    ...
    var touchend = function touchend(event) {
      ...
      var now = Date.now();
      var touch = util$1.hasTouch ? event.changedTouches[0] : event;
      x2 = touch.pageX;
      ...
      util$1._trigger(eventList.SWIPE_END, el, event);
      ...
      if (distanceX <= _config.tapMaxDistance && distanceY <= _config.tapMaxDistance) {
        ...
        if (!el._m_touch_events[eventList.DOUBLE_TAP] || !el._m_touch_events[eventList.DOUBLE_TAP].length) {
          ...
          util$1._trigger(eventList.DOUBLE_TAP, el, event); // 双击后重置最后触屏时间为0，是为了从新开始计算下一次双击时长
        ...
        var direction = util$1.swipeDirection(x1, y1, x2, y2, directionLocked);
        resetTimer(); // 滑动结束时的方向和滑动时方向一样则触发swipexxx事件

        if (direction === moveDirection) {
          util$1._trigger(eventList["SWIPE_".concat(moveDirection)], el, event); // 否则认为是取消滑动
  ...
  var dir = {
    ...
    inserted: function inserted() {
      dir.update.apply(dir, arguments);
    },
    update: function update(el, binding) {
      var event = binding.arg;
      var selector = el.getAttribute("".concat(event, "-selector"));
      var handler = binding.value;

      if (el["_m_touch_handler_".concat(binding.arg)]) {
        el._m_touch.off(event, selector || el["_m_touch_handler_".concat(binding.arg)], el["_m_touch_handler_".concat(binding.arg)]);
      }
      ...
      el["_m_touch_handler_".concat(binding.arg)] = handler;

      el._m_touch.on(event, selector, handler);
    },
    unbind: function unbind(el, binding) {
      el["_m_touch_handler_".concat(binding.arg)] = null;
